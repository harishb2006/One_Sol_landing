...
const LogoMarquee = () => {
  return (
    <div className="bg-[#F4F9FF] py-20 overflow-hidden w-full">
      ...
      <div className="relative flex overflow-x-hidden">
        ...
        >
          {duplicatedLogos.map((logo, index) => (
            <motion.div
              ...
              className="flex items-center justify-center min-w-[250px] h-[100px] mx-4 bg-white border border-gray-100 rounded-xl shadow-sm transition-all duration-300"
              style={{
                filter: 'blur(0px)',
              }}
            >
              <div className="flex items-center space-x-3 text-slate-800">
                <div className="p-2 bg-slate-50 rounded-lg transition-transform">
                  {logo.icon}
                </div>
        ...
        {/* Enhanced Gradient Overlays with Blur Effect - Reduced blur windows */}
        <div className="absolute inset-y-0 left-0 w-[260px] z-10 pointer-events-none" 
             style={{ 
               background: 'linear-gradient(to right, rgba(244, 249, 255, 1) 0%, rgba(244, 249, 255, 0.6) 40%, transparent 100%)',
               backdropFilter: 'blur(4px)'
             }} 
        />
        <div className="absolute inset-y-0 right-0 w-[260px] z-10 pointer-events-none" 
             style={{ 
